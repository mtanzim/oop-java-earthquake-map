...
//        earthquakesURL = "quiz1.atom";
    ...
    private void printQuakes() {
        ...
        // populate the map with earthquake counts
        for (Marker cm : quakeMarkers) {
            if (cm.getProperty("country") != null) curCountry = (String) cm.getProperty("country");
            else curCountry = "OCEAN QUAKE";

            if (!EQMap.containsKey(curCountry)) EQMap.put(curCountry, 1);
            else {
                curCount = EQMap.get(curCountry);
                EQMap.put(curCountry, curCount + 1);
            }

        }


        for (Map.Entry<String, Integer> entry : EQMap.entrySet()) {
